...
class LeftPane(wx.GridBagSizer):
    ...
    def __init__(self, root):
        ...
        # Hotend 1
        self.Add(wx.StaticText(root.panel,-1, _("Heat:")), pos = (2, 0), span = (1, 1), 
            flag = wx.ALIGN_CENTER_VERTICAL|wx.ALIGN_RIGHT)
        htemp_choices = [root.temps[i]+" ("+i+")" for i in sorted(root.temps.keys(), key = lambda x:root.temps[x])]

        root.settoff = make_button(root.panel, _("Off"), lambda e: root.do_settemp("off"), 
            _("Switch Hotend Off"), size = (36,-1), style = wx.BU_EXACTFIT)
		...
		# Hotend 2 
        # Text
        #self.Add(wx.StaticText(root.panel,-1, _("Heat 2:")), pos = (4, 0), span = (1, 1), 
        #    flag = wx.ALIGN_CENTER_VERTICAL|wx.ALIGN_RIGHT)
        # Temps
        #htemp2_choices = [root.temps[i]+" ("+i+")" for i in sorted(root.temps.keys(), key = lambda x:root.temps[x])]
        # Add Off-button
        #root.settoff2 = make_button(root.panel, _("Off"), lambda e: root.do_settemp("off"), _("Switch Hotend Off"), size = (36,-1), style = wx.BU_EXACTFIT)
        #root.printerControls.append(root.settoff2)
        #self.Add(root.settoff2, pos = (4, 1), span = (1, 1)) # Add off-button
        
        #if root.settings.last_temperature2 not in map(float, root.temps.values()):
        #    htemp2_choices = [str(root.settings.last_temperature)] + htemp_choices
        #root.htemp2 = wx.ComboBox(root.panel, -1, choices = htemp2_choices, style = wx.CB_DROPDOWN, size = (80,-1))
        #root.htemp2.SetToolTip(wx.ToolTip("Select Temperature for Hotend 2"))
        #root.htemp2.Bind(wx.EVT_COMBOBOX, root.htemp2_change)

        #self.Add(root.htemp2, pos = (2, 2), span = (1, 2))
        #root.settbtn2 = make_button(root.panel, _("Set"), root.do_settemp, _("Switch Hotend 2 On"), 
        #    size = (38, -1), style = wx.BU_EXACTFIT)
        #root.printerControls.append(root.settbtn2)
        #self.Add(root.settbtn2, pos = (2, 4), span = (1, 1))
        

        root.btemp.SetValue(str(root.settings.last_bed_temperature))
        root.htemp.SetValue(str(root.settings.last_temperature))
        #root.htemp2.SetValue(str(root.settings.last_temperature2))
